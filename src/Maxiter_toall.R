...
# iteration set
Maxiter_all <- c(500,1000,1500,2000,2500,3000,
...
file_name_SVM <- paste0("results/Maxiter/STM_m",m,"_n",n,
                        "_deg",deg,"_c",c,"_rho",rho,"_rp",repeat_time,"_d",prod_d,"_lam",lambda,".xlsx")

# Column names for table
colname_df <- c("repeat time","n","m","prod_d","s","pi1","pi2","Maxiter",
                "lambda","rho","c","deg","error","Nerror",
                "error_to_tall","Q_loss_to_tall","error_to_all","Q_loss_to_all",
                paste0("beta_",1:prod(s)))
result_df <- data.frame(matrix(ncol = length(colname_df), nrow = 0))
colnames(result_df) <- colname_df

# Generate files and update results in real time
if(file.exists(file_name_SVM)){
  message("STM result appended to existing file.")
}else{
  wb <- createWorkbook()  # Create XLSX file
  addWorksheet(wb, "Sheet 1")  # Add a worksheet
  writeData(wb, "Sheet 1", result_df) # enter the column names
  saveWorkbook(wb, file_name_SVM, overwrite = TRUE)  # Save XLSX file
  message("STM file created and data written. ")
}

##### simulations ##############################################################
# Record
repeat_n <- length(Maxiter_all)
error_sum <- Nerror_sum <- matrix(0,repeat_n,repeat_time)
theta_new_sum <- theta_mean <- matrix(0,repeat_n,prod_d*m)

error_to_tall_sum <- Q_loss_to_tall_sum <- matrix(0,repeat_n,repeat_time)
error_to_all_sum <- Q_loss_to_all_sum <- matrix(0,repeat_n,repeat_time)

for(re in 1:repeat_n){
  # iteration number
  Maxiter <- Maxiter_all[re]
  ...
  for(ret in 1:repeat_time){
    ...
    for(tgd in 1:Maxiter){
      ...
      error_all[tgd] <- norm(theta_new_all-beta_plus_star,"2")
      Q_loss_all[tgd] <- loss_function(y,x,theta_new_all,lambda)
    }
    
    ##### Decentralized part ##############################################################
    # Record
    error <- Q_loss <- error_to_tall <- Q_loss_to_tall <- error_to_all <- Q_loss_to_all <- numeric(Maxiter)
    ...
    for(tgd in 1:Maxiter){
      ...
      # Record the error and loss for each iteration
      error[tgd] <- mean(apply(matrix(theta_new,prod_d,m)-beta_plus_star,2,norm,"2"))
      Q_loss[tgd] <- loss_function(y,x,theta_new,lambda)
      
      # Record the error and loss for each iteration
      error_to_tall[tgd] <- mean(apply(matrix(theta_new_tall / tgd,prod_d,m) - theta_new_all,2,norm,"2"))
      Q_loss_to_tall[tgd] <- loss_function(y,x,theta_new_tall / tgd,lambda) - Q_loss_all[Maxiter]
      
      error_to_all[tgd] <- mean(apply(matrix(theta_new,prod_d,m)-theta_new_all,2,norm,"2"))
      Q_loss_to_all[tgd] <- loss_function(y,x,theta_new,lambda) - Q_loss_all[Maxiter]
    }
    
    # # Recording the convergence process of algorithm
    # result_ret <- cbind(error,Q_loss,error_all,Q_loss_all,error_to_tall,Q_loss_to_tall,error_to_all,Q_loss_to_all)
    # filename_ret <- paste0("results/Maxiter/STM_tall_m",m,"_n",log10(n),"_deg",deg,"_rp",repeat_time,"_d",prod_d,"_lam",lambda,"_ret",ret,"_rho",rho,"_c",c,"_Mxi",Maxiter,".csv")
    ...
    error_to_tall_sum[re,ret] <- error_to_tall[tgd]
    Q_loss_to_tall_sum[re,ret] <- Q_loss_to_tall[tgd]
    error_to_all_sum[re,ret] <- error_to_all[tgd]
    Q_loss_to_all_sum[re,ret] <- Q_loss_to_all[tgd]
    
    # Write the results to XLSX file
    SVM_onetime <- c(ret,n,m,prod_d,prod(s),pi1,pi2,Maxiter,lambda,rho,c,deg,error_sum[re,ret],Nerror_sum[re,ret],
                     error_to_tall_sum[re,ret],Q_loss_to_tall_sum[re,ret],error_to_all_sum[re,ret],Q_loss_to_all_sum[re,ret],
                     theta_new[1:prod(s)])
    
    start_row <- repeat_time*(re-1) + ret + 1
    print(start_row - 1)
    
    wb <- loadWorkbook(file_name_SVM)
    writeData(wb, sheet = "Sheet 1", as.data.frame(t(SVM_onetime)), startRow = start_row, colNames = (start_row == 1))
    saveWorkbook(wb, file_name_SVM, overwrite = TRUE)
  }
  # Calculate the mean of the estimated coefficients from repeated experiments
  theta_mean[re,] <- theta_new_sum[re,] / repeat_time
}
# Calculate the mean and variance of predicted error from repeated experiments
error_mean <- apply(error_sum,1,mean)
error_sd <- apply(error_sum,1,sd)
Nerror_mean <- apply(Nerror_sum,1,mean)

error_to_tall_mean <- apply(error_to_tall_sum,1,mean)
Q_loss_to_tall_mean <- apply(Q_loss_to_tall_sum,1,mean)
error_to_all_mean <- apply(error_to_all_sum,1,mean)
Q_loss_to_all_mean <- apply(Q_loss_to_all_sum,1,mean)

# Write the summary results to CSV file
colname_df_mean <- c("repeat time","n","m","d","s","pi1","pi2","Maxiter","lambda","rho","c","deg","error","error_sd","Nerror",
                     "error_to_tall","Q_loss_to_tall","error_to_all","Q_loss_to_all",
                     paste0("beta_",diag_positions))

SVM_all <- cbind(repeat_time,n,m,prod_d,prod(s),pi1,pi2,Maxiter_all,lambda,rho,c,deg,error_mean,error_sd,Nerror_mean,
                 error_to_tall_mean,Q_loss_to_tall_mean,error_to_all_mean,Q_loss_to_all_mean,
                 theta_mean[,diag_positions])
result_df_mean <- as.data.frame(SVM_all)
colnames(result_df_mean) <- colname_df_mean

filename <- paste0("results/Maxiter/All_STM_m",m,"_n",n,"_deg",deg,"_c",c,"_rho",rho,"_rp",repeat_time,"_d",prod_d,"_lam",lambda,".csv")
write.csv(result_df_mean,filename)
