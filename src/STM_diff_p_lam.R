...
file_name_SVM <- paste0("results/dn/STM_m",m,"_n_",n_min,"to",n_max,"by",n_interval,
                        "_deg",deg,"_c",c,"_rho",rho,"_rp",repeat_time,"_d",prod_d,"_lam",lambda,"_Mxi",Maxiter,".xlsx")

# Column names for table
colname_df <- c("repeat time","n","m","prod_d","s","pi1","pi2","Maxiter",
                "lambda","rho","c","deg","error","Nerror",paste0("beta_",1:(d[1]*d[2]*s[3])))
...
for(re in 1:repeat_n){
  ...
  for(ret in 1:repeat_time){
    ...
    SVM_onetime <- c(ret,n,m,prod_d,prod(s),pi1,pi2,Maxiter,lambda,rho,c,deg,error_all[re,ret],Nerror_all[re,ret],
                     theta_new[1:(d[1]*d[2]*s[3])])
...
SVM_all <- cbind(repeat_time,n_all,m,prod_d,prod(s),pi1,pi2,Maxiter,lambda,rho,c,deg,error_mean,error_sd,Nerror_mean,
                 theta_mean[,diag_positions])
result_df_mean <- as.data.frame(SVM_all)
colnames(result_df_mean) <- colname_df_mean

filename <- paste0("results/dn/All_STM_m",m,"_n_",n_min,"to",n_max,"by",n_interval,"_deg",deg,"_c",c,"_rho",rho,"_rp",repeat_time,"_d",prod_d,"_lam",lambda,"_Mxi",Maxiter,".csv")
