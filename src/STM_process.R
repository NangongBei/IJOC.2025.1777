...
rho_all <- 0.5 # c(0.001,0.01,0.1,0.5,1,1.5) # 
c_all <- c(10,50,100,500,1000,1) # 10 # 
Maxiter <- 2000
...
if((length(c_all) > 1)&(length(rho_all) == 1)){
  rho <- rho_all
  file_name_SVM <- paste0("results/process/dc_STM_m",m,"_n",log10(n),"_deg",deg,"_rho",rho,
                          "_rp",repeat_time,"_d",prod_d,"_lam",lambda,"_Mxi",Maxiter,".xlsx")
  filename <- paste0("results/dc_All_STM_m",m,"_n",log10(n),"_deg",deg,"_rho",rho,
                     "_rp",repeat_time,"_d",prod_d,"_lam",lambda,"_Mxi",Maxiter,".csv")
  
}else if((length(rho_all) > 1)&(length(c_all) == 1)){
  c <- c_all
  file_name_SVM <- paste0("results/process/drho_STM_m",m,"_n",log10(n),"_deg",deg,"_c",c,
                          "_rp",repeat_time,"_d",prod_d,"_lam",lambda,"_Mxi",Maxiter,".xlsx")
  filename <- paste0("results/process/drho_All_STM_m",m,"_n",log10(n),"_deg",deg,"_c",c,
                     "_rp",repeat_time,"_d",prod_d,"_lam",lambda,"_Mxi",Maxiter,".csv")
  ...
  file_name_SVM <- paste0("results/process/STM_m",m,"_n",log10(n),"_deg",deg,"_c",c,"_rho",rho,
                          "_rp",repeat_time,"_d",prod_d,"_lam",lambda,"_Mxi",Maxiter,".xlsx")
  filename <- paste0("results/process/All_STM_m",m,"_n",log10(n),"_deg",deg,"_c",c,"_rho",rho,
                     "_rp",repeat_time,"_d",prod_d,"_lam",lambda,"_Mxi",Maxiter,".csv")
  
}else if((length(rho_all) > 1)&(length(c_all) > 1)){
  stop("Please fix at least one of c or rho to observe the effect of the step size on the algorithm's convergence process.")
}

# Column names for table
colname_df <- c("repeat time","n","m","prod_d","s","pi1","pi2","Maxiter",
                "lambda","rho","c","deg","error","Nerror",paste0("beta_",1:prod(s)))
...
##### simulations ##############################################################
# Record
repeat_n <- max(length(c_all),length(rho_all))
theta_new_all <- theta_mean <- matrix(0,repeat_n,prod_d*m)
error_all <- Nerror_all <- matrix(0,repeat_n,repeat_time)

for(re in 1:repeat_n){
  # Step size
  if(length(c_all) > 1){
    c <- c_all[re]
  }else if(length(rho_all) > 1){
    rho <- rho_all[re]
  }
  ...
  for(ret in 1:repeat_time){
    ...
    # Record
    error <- Q_loss <- numeric(Maxiter)
    theta_new <- theta_old <- u1_new <- v2_new <- w3_new <- 
    ...
    for(tgd in 1:Maxiter){
      ...
      theta_m_mean <- apply(matrix(theta_new,prod_d,m),1,mean)
      ...
      Q_loss[tgd] <- loss_function(y,x,theta_m_mean,lambda)
      
      # exit the iteration if diverge
      if(error[tgd] > 1e10){
        break
      }
    }
    ...
    if((log10(n) == round(log10(n))) & (ret == 1)){
      result_ret <- cbind(error[1:tgd],Q_loss[1:tgd])
      colnames(result_ret) <- c("error","Q_loss")
      filename_ret <- paste0("results/process/STM_onetime_m",m,"_n",log10(n),"_deg",deg,"_rp",repeat_time,"_d",prod_d,"_lam",lambda,"_ret",ret,"_rho",rho,"_c",c,"_Mxi",Maxiter,".csv")
      write.csv(result_ret,filename_ret)
    }
    
    # Record the coefficient estimates and errors for each iteration
    theta_new_all[re,] <- theta_new_all[re,] + theta_new
    error_all[re,ret] <- error[tgd]
    Nerror_all[re,ret] <- error[tgd] / norm2_beta_star
    
    # Write the results to XLSX file
    SVM_onetime <- c(ret,n,m,prod_d,prod(s),pi1,pi2,tgd,lambda,rho,c,deg,error_all[re,ret],Nerror_all[re,ret],
                     theta_new[1:prod(s)])
  ...
  theta_mean[re,] <- theta_new_all[re,] / repeat_time
}
# Calculate the mean and variance of predicted error from repeated experiments
error_mean <- apply(error_all,1,mean)
error_sd <- apply(error_all,1,sd)
Nerror_mean <- apply(Nerror_all,1,mean)

# Write the summary results to CSV file
colname_df_mean <- c("repeat time","n","m","d","s","pi1","pi2","Maxiter","lambda","rho","c","deg","error","error_sd",
                     "Nerror",paste0("beta_",diag_positions))

SVM_all <- cbind(repeat_time,n,m,prod_d,prod(s),pi1,pi2,Maxiter,lambda,rho_all,c_all,deg,error_mean,error_sd,Nerror_mean,
