...
n <- 30 # Sample size
m_all <- c(11,20,50,100,200,400) # nodes number set
deg <- 5 # Connectivity
...
for(re in 1:repeat_n){
  ...
  for(ret in 1:repeat_time){
    ...
    filename_ret <- paste0("results/process/STM_tall_m",m,"_n",n,"_deg",deg,"_rp",repeat_time,"_d",prod_d,"_lam",lambda,"_ret",ret,"_rho",rho,"_c",c,"_Mxi",Maxiter,".csv")
